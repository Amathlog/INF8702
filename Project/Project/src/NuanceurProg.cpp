...
{	
   ...
   if (nsStr != "") {
      ...
      ns = readFile(nsStr);
      const char* nsParsed = ns.c_str();

      // sourcer le nuanceur
      glShaderSource(nuanceurSommets, 1, &nsParsed, NULL);

      glCompileShader(nuanceurSommets);
      afficherShaderInfoLog(nuanceurSommets, "VERTEX SHADER COMPILATION ISSUES : ");
   }
   ...
   if (nfStr != "") {
      ...
      // lecture du code du nuanceur
      nf = readFile(nfStr);
      const char* nfParsed = nf.c_str();

      // sourcer le nuanceur
      glShaderSource(nuanceurFragments, 1, &nfParsed, NULL);

      glCompileShader(nuanceurFragments);
      afficherShaderInfoLog(nuanceurFragments, "FRAGMENT SHADER COMPILATION ISSUES : ");
   }
