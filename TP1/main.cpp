...
void initialiserCube(void)
{
	...
	// Nombre d'éléments dans indices sommets
	tailleCube = sizeof(indices_sommets) / sizeof(unsigned int);
...
void dessinerCube(void)
{
	...
	glm::mat4 translationMatrix = glm::translate(translate);
	...
	if (CVar::isRotating)
	{
		...
		rotationMatrix = glm::rotate(static_cast<float>(0.75f * CVar::temps), rotation);
	}

	// Caluler la matrice modèle (attention à l'ordre de multiplcations.:
	glm::mat4 modelView = translationMatrix * rotationMatrix * scaleMatrix;

	// Activer le nuanceur approprié
	progNuanceurCube.activer();

	// Fournir au nuanceur la matrice mvp:
	GLuint handle;

	handle = glGetUniformLocation(progNuanceurCube.getProg(), "M");
	glUniformMatrix4fv(handle, 1, GL_FALSE, &modelView[0][0]);

	handle = glGetUniformLocation(progNuanceurCube.getProg(), "V");
	glUniformMatrix4fv(handle, 1, GL_FALSE, &CVar::vue[0][0]);

	handle = glGetUniformLocation(progNuanceurCube.getProg(), "P");
