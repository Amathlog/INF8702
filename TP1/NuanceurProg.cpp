...
void CNuanceurProg::compilerEtLierNuanceurs(char* nsStr, char* nfStr)
{
	...
	if (nsStr) {
		...
		const char* constantNs = ns;

		// sourcer le nuanceur
		glShaderSource(nuanceurSommets, 1, &constantNs, NULL);
	...
	if (nfStr) {
		...
		// créer un pointeur constant sur le texte du code du nuanceur
		const char* constantNf = ns;

		// sourcer le nuanceur
		glShaderSource(nuanceurFragments, 1, &constantNf, NULL);
		...
		// Afficher les infos:
		afficherShaderInfoLog(nuanceurFragments, "ERREURS DE COMPILATION DU NUANCEUR DE SOMMETS : ");
	}

	// créer le programme des nuanceurs et lier
	prog_ = glCreateProgram();

	// Attache les nuanceurs (s'ils ont été créés)
	if (nsStr)
	...
	//// Clean up, on détache les nuanceurs
	//if (nsStr)
	//	glDetachShader(prog_, nuanceurSommets);
	//if (nfStr)
	//	glDetachShader(prog_, nuanceurFragments);
}
